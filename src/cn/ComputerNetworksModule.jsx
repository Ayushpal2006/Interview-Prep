...
import {
  Network, Search, Sparkles, BookOpen, Code2, HelpCircle,
  CheckCircle2, AlertTriangle, ChevronDown, Flame, ShieldCheck, Clock
} from "lucide-react";
import { TOPICS, QUICK_REVISION, RAPID_FIRE, QUESTION_BANK } from "./cnData";
import {
  GoogleRequestFlowDiagram,
  OSITCPIPVisuals,
  ComparisonMatrixSection,
  TopologiesVisualizer,
  CastingVisualizer
} from "./CnVisuals";
...
/* ======================================================================
   INDIVIDUAL CN TOPIC CARD COMPONENT
   ====================================================================== */
function TopicCard({ topic }) {
  ...
  return (
    <div
      id={topic.id}
      className="rounded-3xl p-5 sticker mb-6 transition-all"
      style={{ backgroundColor: C.card, border: `3.5px solid ${INK}` }}
    >
      {/* HEADER */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3">
        <div className="flex items-center flex-wrap gap-2">
          <span
            ...
            style={{
              backgroundColor: topic.importance === "HIGH FREQUENCY" ? C.coral : C.sun,
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs font-display font-extrabold text-amber-700 hover:underline"
        >
      ...
      {/* SHORT CONCEPT */}
      <p className="text-xs font-ui text-gray-700 leading-relaxed mb-4 bg-slate-50 p-3 rounded-2xl border border-slate-200">
        ...
        <div className="space-y-4">
          ...
          <div className="p-3.5 rounded-2xl" style={{ backgroundColor: C.paper, border: `2px solid ${INK}` }}>
            <p className="font-display text-xs font-bold text-gray-900 mb-2 flex items-center gap-1.5">
              <Sparkles size={14} className="text-amber-600" /> Key Points to Remember
            </p>
          ...
          {topic.code && (
            <div className="rounded-2xl p-3.5 bg-slate-900 border-2 border-slate-800 text-slate-100 font-code text-xs">
              <p className="text-amber-400 font-bold mb-1">// Protocol Format / Execution Flow:</p>
...
/* ======================================================================
   MAIN COMPUTER NETWORKS MODULE CONTAINER
   ====================================================================== */
export default function ComputerNetworksModule() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-3xl p-6 sticker text-slate-900" style={{ backgroundColor: C.sun, border: `3.5px solid ${INK}` }}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="px-3 py-1 rounded-full text-xs font-bold font-code bg-slate-900 text-amber-300">
                Cover-To-Cover CN Reference
              </span>
              <span className="px-3 py-1 rounded-full text-xs font-bold font-code bg-white text-slate-900 border border-slate-900">
                36 Networking Topics
              </span>
            </div>
            <h1 className="font-display text-2xl sm:text-3xl font-black">
              Computer Networks (CN) Interview Preparation System
            </h1>
            <p className="text-xs font-ui opacity-90 mt-1 max-w-2xl">
              Complete interview preparation module covering Google Request Flow, OSI 7-Layers, TCP/IP, IPv4 Classes, Subnetting, Protocols, Hub vs Switch, MAC vs IP, Firewalls &amp; Casting Modes.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl p-4 sticker bg-white" style={{ border: `3px solid ${INK}` }}>
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-4 pb-3 border-b-2 border-slate-100">
          ...
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            {[
              { id: "full", label: "🌐 Complete CN Module", color: C.amber },
          ...
          <div className="relative w-full md:w-64">
            ...
            <input
              type="text"
              placeholder="Search CN topics or Qs..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-9 pr-4 py-2 rounded-2xl text-xs font-ui border-2 border-slate-900 focus:outline-none focus:ring-2 focus:ring-amber-400"
            />
          </div>
        </div>

        {/* DIFFICULTY FILTER (IF FULL MODE) */}
        {viewMode === "full" && (
      ...
      {/* MODE 1: COMPLETE TOPICS VIEW */}
      {viewMode === "full" && (
        <>
          {/* EMPHASIZED GOOGLE REQUEST FLOW SPOTLIGHT */}
          <GoogleRequestFlowDiagram />

          {/* LAYER ARCHITECTURE & COMPARISON MATRICES */}
          <OSITCPIPVisuals />
          <ComparisonMatrixSection />
          <TopologiesVisualizer />
          <CastingVisualizer />
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display text-xl font-extrabold flex items-center gap-2" style={{ color: INK }}>
              <Network size={22} className="text-amber-600" /> Networking Topic Breakdown ({filteredTopics.length} Topics)
            </h2>
          </div>
        ...
        <div className="rounded-3xl p-6 sticker bg-white" style={{ border: `3.5px solid ${INK}` }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-display text-2xl font-extrabold" style={{ color: INK }}>
              COMPUTER NETWORKS — 5 MINUTE QUICK REVISION
            </h2>
          </div>
          <p className="text-xs font-ui text-gray-600 mb-6">
            Scannable summary cards covering all 36 networking terms from your placement notes.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {QUICK_REVISION.map((item, idx) => (
              <div
                ...
                style={{ backgroundColor: C.paper, border: `2px solid ${INK}` }}
              >
                <p className="font-display text-sm font-black text-amber-900 mb-1">{item.term}</p>
      ...
      {viewMode === "rapid" && (
        <div className="rounded-3xl p-6 sticker bg-white" style={{ border: `3.5px solid ${INK}` }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-display text-2xl font-extrabold" style={{ color: INK }}>
              RAPID FIRE — NETWORKING FLASHCARDS
            </h2>
          </div>
          <p className="text-xs font-ui text-gray-600 mb-6">
            High-frequency rapid-fire facts for self-testing ports, protocols, and layers.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {RAPID_FIRE.map((item, idx) => (
              <div
                ...
                style={{ backgroundColor: C.paper, border: `2.5px solid ${INK}` }}
              >
                <p className="font-display text-sm font-extrabold text-slate-900 mb-2">
                  Q{idx + 1}: {item.q}
                </p>
      ...
      {viewMode === "qbank" && (
        <div className="rounded-3xl p-6 sticker bg-white" style={{ border: `3.5px solid ${INK}` }}>
          ...
          <p className="text-xs font-ui text-gray-600 mb-6">
            Substantial question collection for Computer Networks SDE interviews.
          </p>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      <span
                        className="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-code"
                        style={{
                          backgroundColor:
                            q.difficulty === "Easy"
                              ? "#DCFCE7"
                      ...
                      <span
                        className="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-code bg-slate-100 text-slate-800"
                        style={{ border: `1.5px solid ${INK}` }}
                      >
                        {q.category}
                      </span>
                    ...
                    <button
                      onClick={() => toggleQBankAnswer(q.id)}
                      className="text-xs font-display font-bold text-amber-700 hover:underline"
                    >
                      {isOpen ? "Hide Answer" : "Show Answer"}
                    </button>
                  </div>

                  <p className="font-display text-sm font-extrabold text-slate-900 mb-2 whitespace-pre-line">
                    {q.question}
                  </p>
                  ...
                  {isOpen && (
                    <div className="mt-3 p-3 rounded-xl bg-slate-900 text-slate-100 font-ui text-xs space-y-2 border border-slate-800">
                      ...
                      {q.followUp && (
                        <p className="text-amber-300 font-code text-[11px] pt-1 border-t border-slate-700">
